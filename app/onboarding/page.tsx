...
const ACTIVITIES: { value: Activity; label: string; desc: string }[] = [
  ...
  { value: "moderate", label: "Gym 2-3x per week", desc: "Regular exercise" },
...
export default function OnboardingPage() {
  ...
  async function handleFinish() {
    setSaving(true)
    const finalProfile: UserProfile = { ...(profile as UserProfile), dailyCalorieGoal: getCalGoal() }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 bg-[#f6fbf8]">
      <div className="w-full max-w-lg">

        <a href="/" className="flex items-center gap-2 mb-10 justify-center" style={{textDecoration:"none"}}>
          ...
          <span className="text-green-800 font-extrabold text-xl">TrimTrack</span>
        </a>
        ...
        <p className="text-xs text-green-700 font-semibold mb-6 text-center uppercase tracking-wide">
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            <motion.div key="s0" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}>
              <div className="card">
                <h2 className="text-2xl font-extrabold text-gray-900 mb-1">Let us set up your plan</h2>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <input className="input-base" type="number" placeholder="28" value={profile.age}
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}>
              <div className="card">
                    ...
                    {ACTIVITIES.map(a => (
                      <button key={a.value} onClick={() => update("activity", a.value)}
                        className={"text-left px-4 py-3 rounded-2xl border transition-all " + (
                          profile.activity === a.value ? "bg-green-700 border-green-700 text-white" : "border-gray-200 hover:bg-gray-50"
                        )}>
                ...
                <div className="mb-5">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">Foods you do not eat?</label>
                  <div className="flex flex-wrap gap-2">
                    {AVOID_OPTIONS.map(a => (
                      <button key={a} onClick={() => toggleAvoid(a)}
                        className={"pill " + (
                          (profile.avoidFoods ?? []).includes(a) ||
                          (a === "No restrictions" && (profile.avoidFoods ?? []).length === 0) ? "pill-active" : ""
                        )}>{a}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => update("reminders", true)}
                      className={"pill " + (profile.reminders ? "pill-active" : "")}>Yes - morning and evening</button>
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}>
              <div className="card">
                <h2 className="text-2xl font-extrabold text-gray-900 mb-1">Your plan is ready</h2>
                <p className="text-sm text-gray-500 mb-6">Here is what TrimTrack calculated for you.</p>
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {([
                    ...
                    { label: "Meal culture", val: profile.country || "Global" },
                  ] as { label: string; val: string; highlight?: boolean }[]).map(s => (
                    <div key={s.label} className={"rounded-2xl p-4 " + (s.highlight ? "bg-green-700" : "bg-green-50")}>
                      <div className={"text-xs font-semibold uppercase mb-1 " + (s.highlight ? "text-green-100" : "text-gray-500")}>{s.label}</div>
                      <div className={"text-xl font-extrabold " + (s.highlight ? "text-white" : "text-gray-900")}>{s.val}</div>
                    </div>
                  ))}
                </div>
                <div className="text-sm text-gray-600 space-y-1">
                  <div>Reminders: <strong>{profile.reminders ? "Morning and evening" : "Off"}</strong></div>
          ...
          {step === 4 && (
            <motion.div key="s4" initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} exit={{opacity:0}}>
              <div className="card text-center">
                <div className="w-20 h-20 bg-lime-400 rounded-full flex items-center justify-center mx-auto mb-6 text-3xl font-bold text-green-900">
                  {profile.name ? profile.name.charAt(0).toUpperCase() : "T"}
                </div>
                <h2 className="text-2xl font-extrabold text-gray-900 mb-2">
                  Your plan is ready, {profile.name || "friend"}!
                </h2>
                <p className="text-sm text-gray-400 mb-6">Here is what TrimTrack calculated for you</p>
                <div className="bg-green-700 rounded-3xl p-6 mb-4 text-center">
                  ...
                  <div className="text-6xl font-extrabold text-white">{getCalGoal().toLocaleString()}</div>
                  <div className="text-green-200 text-sm mt-1">kcal per day</div>
                </div>
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-green-50 rounded-2xl p-4 text-left">
                    <div className="text-xs text-gray-400 mb-1">To lose</div>
                  ...
                  <div className="bg-green-50 rounded-2xl p-4 text-left">
                    ...
                    <div className="text-2xl font-extrabold text-gray-900">{getWeeks()} weeks</div>
                  </div>
                  <div className="bg-green-50 rounded-2xl p-4 text-left">
                    <div className="text-xs text-gray-400 mb-1">Meal culture</div>
                    <div className="text-lg font-extrabold text-gray-900">{profile.country || "Global"}</div>
                  </div>
                  <div className="bg-green-50 rounded-2xl p-4 text-left">
                    <div className="text-xs text-gray-400 mb-1">Reminders</div>
                    <div className="text-lg font-extrabold text-gray-900">
                      {profile.reminders ? "Morning + evening" : "Off"}
                    </div>
                  </div>
                </div>
                <div className="bg-lime-50 border border-lime-200 rounded-2xl px-4 py-3 mb-4">
                ...
                <p className="text-xs text-gray-400">TrimTrack understands your food.</p>
              </div>
            </motion.div>
          )}
