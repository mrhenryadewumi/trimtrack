    ...
    <div>
      ...
      <div className="flex gap-2 mb-3">
        <div className="flex-1 relative">
          <input
            className="w-full border border-white/10 rounded-full px-4 py-2.5 text-sm outline-none focus:border-[#b5f23d] bg-[#0e1e16] text-white placeholder-[#5f7269] pr-10"
          ...
          {loading && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2">
              <div className="w-4 h-4 border-2 border-[#b5f23d] border-t-transparent rounded-full animate-spin"/>
            </div>
          )}
        </div>
        {/* BARCODE BUTTON */}
        <button
          ...
          className="w-10 h-10 bg-[#b5f23d] text-[#0a1310] rounded-full flex items-center justify-center hover:bg-[#8dc42a] flex-shrink-0"
          title="Scan barcode"
        >
      ...
      {query.length >= 2 && !loading && (
        <div className="text-xs text-[#5f7269] mb-2 px-1">
          {results.filter(r => r.source === 'local').length > 0 && (
            <span className="text-[#b5f23d] font-semibold">Nigerian foods · </span>
          )}
      ...
      <div className="space-y-1.5 max-h-64 overflow-y-auto">
        {results.map((food) => (
          <motion.div
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-white/5 bg-[#0e1e16] hover:bg-[#1f3a2b] cursor-pointer transition-colors"
          ...
          >
            ...
            {!food.image && (
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 bg-[#233020] text-[#b5f23d]">
                {food.source === 'local' ? 'NG' : 'GL'}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-white truncate">{food.name}</div>
              <div className="text-xs text-[#5f7269] truncate">
                {food.brand && <span className="text-[#8a9a92]">{food.brand} · </span>}
                {food.protein}g P · {food.carbs}g C · {food.fat}g F
                {food.serving && <span className="text-[#3f4a45]"> · {food.serving}</span>}
              </div>
            </div>
            <div className="flex-shrink-0 text-right">
              <div className="text-sm font-extrabold text-[#b5f23d]">{food.kcal}</div>
              <div className="text-xs text-[#5f7269]">kcal</div>
            </div>
          </motion.div>
        ))}
        ...
        {query.length >= 2 && !loading && results.length === 0 && (
          <div className="text-center py-6">
            <div className="text-[#8a9a92] text-sm mb-1">No results for "{query}"</div>
            <div className="text-xs text-[#5f7269]">Try scanning the barcode instead</div>
          </div>
        )}
      </div>
